...
class MARLTippingPointEnv(MultiAgentEnv):
    ...
    def __init__(self, steps, num_agents):
        super(MARLTippingPointEnv, self).__init__()
    ...
    def step(self, action_list):
        ...
        agent_list = [None] * self.num_agents
        if self.num_agents == 2:
            agent_list[0] = [
                robot for robot in field_rep.robots if type(robot) is HostRobot
            ][0]
            agent_list[1] = [
                robot for robot in field_rep.robots if type(robot) is OpposingRobot
            ][0]
        else:
            agent_list = field_rep.robots

        for i, action in enumerate(action_list):
            adjacent_entities = self._detect_adjacents(agent_list[i], field_rep)
            self._do_action(action, agent_list[i], adjacent_entities, field_rep)
