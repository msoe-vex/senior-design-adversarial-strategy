...
def basic_goal_representation():
    goal_arr = [
        ...
        LowNeutralGoal(Pose2D(-35, 72)),  # Top Neutral
        HighNeutralGoal(Pose2D(0, 72)),  # Mid Neutral
        LowNeutralGoal(Pose2D(35, 72)),  # Bottom Neutral
    ]
...
def ending_representation():
    ...
    goal_arr = [
        RedGoal(
            ...
            ring_containers={
                GoalLevel.BASE: RingContainer(
                    rings=[Ring(Pose2D(25, -63)), Ring(Pose2D(25, -63))]
                ),
        ...
        RedGoal(
            Pose2D(-49, 18),
            ring_containers={
                GoalLevel.BASE: RingContainer(rings=[Ring(Pose2D(-49, 18))])
            },
        ),
        LowNeutralGoal(
            Pose2D(-21, 38),
            ring_containers={
                GoalLevel.BASE: RingContainer(
                    rings=[Ring(Pose2D(-21, 38)), Ring(Pose2D(-21, 38))]
                ),
                GoalLevel.LOW: RingContainer(
                    rings=[
                        Ring(Pose2D(-21, 38)),
        ...
        LowNeutralGoal(Pose2D(0, 72), tipped=True),
    ]
    ...
    red_plat = RedPlatform(
        ...
        robots=[
            HostRobot(
                Color.RED,
                Pose2D(0, 4),
                rings=[Ring(Pose2D(0, 4)), Ring(Pose2D(0, 4)), Ring(Pose2D(0, 4))],
                goals=[
                    HighNeutralGoal(
                        Pose2D(0, 4),
                        ring_containers={
    ...
    blue_plat = BluePlatform(
        ...
        robots=[
            OpposingRobot(Color.BLUE, Pose2D(-10, 142)),
            OpposingRobot(
                Color.BLUE, Pose2D(14, 141), goals=[BlueGoal(Pose2D(14, 141))]
            ),
        ],
    )

    robot_arr = [PartnerRobot(Color.RED, Pose2D(-49, 71), tipped=True)]
